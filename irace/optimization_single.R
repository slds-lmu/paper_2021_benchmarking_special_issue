     ...
     private = list(
      .eval_many = function(xss) {
        ...
        eval = function(xs, instance) {
          ...
          archive = if (highest_budget_only) {
            domain = instance$objective$domain
            param_ids = domain$ids()
            budget_idx = which(domain$tags %in% c("budget", "fidelity"))
            budget_id = param_ids[budget_idx]
            instance$archive$data[get(budget_id) == max(get(budget_id)), ]
          } else {
        ...
        res = future.apply::future_mapply(eval, xss, self$irace_instance, future.seed = 7345)

        data.table(y = res["y", ], time = res["time", ], id_plan = self$irace_instance[[1]]$id_plan)
      }
    )
  )
