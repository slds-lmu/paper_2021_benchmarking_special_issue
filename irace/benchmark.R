...
smash_it = function(mu) {
  xs = list(budget_log_step = 0.0646, mu = mu, sample = "random", survival_fraction = 0.2767,  # mu = 4.6069
...
# call smashy with varying mu on same instance
res = future.apply::future_mapply(smash_it, list(1, 2, 3, 4))

# var
highest_budget_only = FALSE
objective_multiplicator = res[[1]]$objective_multiplicator * -1
cols_y = res[[1]]$archive$cols_y

# get archive data and optionally filter for experiments with highest budget only
archives = map(res, function(inst) {
