...
get_meta_objective <- function(objective, test_objective, search_space, budget_limit, highest_budget_only = TRUE, nadir = 0) {
  ...
  function(...) {
    ...
    cols <- c(oi$objective$codomain$ids(), oi$search_space$ids(tags = "budget"))
    ...
    res <- oi$archive$data[, cols, with = FALSE]
    res[, (oi$objective$codomain$ids()) := om* get(oi$objective$codomain$ids())]
    res[, eval_nr := seq_len(.N)]
    res[, best := map_dbl(seq_len(NROW(res)), function(i) min(res[[oi$objective$codomain$ids()]][1:i]))]
    res
